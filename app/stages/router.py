...
@router.get("/{id}", response_model=SStages, summary="Получить материал по ID")
async def get_material(id: str) -> SStages:
    result = await StagesDAO.find_one_or_none(_id=ObjectId(id))
    return result
...
async def add_adress(data: SStagesAdd):
    """
    ...
    try:
        ...
        if not await StagesDAO.is_unique(
                field_name="name",
                value=data.name,
        ...
        result = await StagesDAO.add(document=material_data)
...
async def update_material(
    ...
    try:
        ...
        existing_material = await StagesDAO.find_one_or_none(_id=ObjectId(id))
        ...
        result = await StagesDAO.update_by_id(
            object_id=id,
...
@router.delete(
    "/{id}",
    response_model=Optional[SStages],
...
async def safe_delete_material(
    ...
    try:
        # Проверяем существование материала
        material = await StagesDAO.find_one_or_none(_id=ObjectId(id))
        if not material:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Материал не найден"
        ...
        result = await StagesDAO.soft_delete(id)
